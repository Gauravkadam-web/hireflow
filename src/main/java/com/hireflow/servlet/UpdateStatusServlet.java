...
public class UpdateStatusServlet extends HttpServlet {
    ...
    private static final Set<String> VALID_STATUSES = Set.of("applied", "reviewed", "hired", "rejected");
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse res)
        ...
        if (appIdStr == null || status == null || !VALID_STATUSES.contains(status)) {
            res.sendRedirect(redirect + "?error=Invalid+status+update+request.");
            return;
        }
        ...
        try {
            ...
            if (updated) {
                res.sendRedirect(redirect + "?success=Application+status+updated+to+" + status + ".");
            } else {
                res.sendRedirect(redirect + "?error=Could+not+update+status.+Application+not+found.");
            }
