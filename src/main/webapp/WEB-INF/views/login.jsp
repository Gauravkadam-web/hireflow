...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    .left-tagline{font-size:1.8rem;font-family:'Syne',sans-serif;font-weight:800;color:var(--text);line-height:1.22;letter-spacing:-0.5px;margin-top:36px;}
    .left-tagline em{color:var(--teal);font-style:normal;}
    .left-sub{color:var(--text-2);font-size:0.875rem;margin-top:12px;line-height:1.7;}

    .feature-list{margin-top:28px;display:flex;flex-direction:column;gap:11px;}
    .feature{display:flex;align-items:center;gap:11px;}
    .check{width:20px;height:20px;border-radius:50%;background:var(--teal-glow);border:1px solid rgba(0,212,170,0.25);display:flex;align-items:center;justify-content:center;flex-shrink:0;}
    .check svg{width:10px;height:10px;stroke:var(--teal);stroke-width:2.5;fill:none;}
    .feature span{font-size:0.83rem;color:var(--text-2);}
    .feature strong{color:var(--text);font-weight:500;}

    .stats-strip{margin-top:28px;display:flex;border:1px solid var(--border);border-radius:14px;overflow:hidden;}
    .stat-cell{flex:1;padding:13px 14px;text-align:center;border-right:1px solid var(--border);}
    ...
    .testimonial{margin-top:20px;background:rgba(255,255,255,0.025);border:1px solid var(--border);border-radius:14px;padding:16px 18px;}
    .testimonial-quote{font-size:0.8rem;color:var(--text-2);line-height:1.6;font-style:italic;}
    .testimonial-quote::before{content:'\201C';color:var(--teal);font-size:1.1rem;font-style:normal;margin-right:2px;}
    .t-author{display:flex;align-items:center;gap:10px;margin-top:12px;}
    .t-avatar{width:28px;height:28px;border-radius:50%;background:linear-gradient(135deg,var(--teal),#00a8ff);display:flex;align-items:center;justify-content:center;font-size:0.68rem;font-weight:700;color:#000;flex-shrink:0;}
    .t-name{font-size:0.78rem;font-weight:500;color:var(--text);}
    .t-role{font-size:0.68rem;color:var(--text-3);}

    /* Right panel */
    .card{width:400px;flex-shrink:0;background:var(--navy-2);padding:44px 40px;display:flex;flex-direction:column;justify-content:center;}
    .card-title{font-family:'Syne',sans-serif;font-size:1.3rem;font-weight:700;margin-bottom:4px;}
    .subtitle{color:var(--text-2);font-size:0.85rem;margin-bottom:28px;}

    .alert{padding:11px 14px;border-radius:10px;font-size:0.85rem;margin-bottom:20px;}
    ...
    input[type="email"],input[type="password"],input[type="text"]{width:100%;padding:11px 16px;background:var(--glass);border:1px solid var(--border);border-radius:10px;font-size:0.875rem;font-family:inherit;color:var(--text);transition:all 0.2s;outline:none;}
    ...
    .check-label{display:flex;align-items:center;gap:8px;cursor:pointer;font-size:0.8rem;color:var(--text-2);user-select:none;}
    .check-label input[type="checkbox"]{width:15px;height:15px;accent-color:var(--teal);cursor:pointer;flex-shrink:0;}
    .forgot{font-size:0.8rem;color:var(--teal);text-decoration:none;opacity:0.8;}
    ...
    .divider{display:flex;align-items:center;gap:12px;margin:20px 0;color:var(--text-3);font-size:0.78rem;}
    ...
    .bottom-link{text-align:center;margin-top:20px;font-size:0.82rem;color:var(--text-2);}
    .bottom-link a{color:var(--teal);font-weight:600;text-decoration:none;}
    .bottom-link a:hover{text-decoration:underline;}

    @media(max-width:720px){.wrapper{flex-direction:column;max-width:440px;margin:16px;}.left-panel{display:none;}.card{width:100%;padding:32px 24px;}}
  </style>
</head>
<body>
...
<div class="wrapper">
  <div class="left-panel">
    ...
    <div class="left-tagline">Your next <em>dream role</em><br>is one click away</div>
    <div class="left-sub">Connect with top employers, track applications, and land the job you deserve — all in one place.</div>

    <div class="feature-list">
      <div class="feature">
        <div class="check"><svg viewBox="0 0 12 10"><polyline points="1,5 4,8 11,1"/></svg></div>
        <span><strong>4,200+ active listings</strong> updated daily</span>
      </div>
      <div class="feature">
        <div class="check"><svg viewBox="0 0 12 10"><polyline points="1,5 4,8 11,1"/></svg></div>
        <span><strong>Track every application</strong> in real time</span>
      </div>
      <div class="feature">
        <div class="check"><svg viewBox="0 0 12 10"><polyline points="1,5 4,8 11,1"/></svg></div>
        <span><strong>86% placement rate</strong> — get hired faster</span>
      </div>
    </div>

    <div class="stats-strip">
      <div class="stat-cell"><div class="stat-num">4.2K+</div><div class="stat-lbl">Active Jobs</div></div>
      <div class="stat-cell"><div class="stat-num">1,340</div><div class="stat-lbl">Companies</div></div>
      <div class="stat-cell"><div class="stat-num">86%</div><div class="stat-lbl">Placement</div></div>
    </div>

    <div class="testimonial">
      <div class="testimonial-quote">Got hired at a top MNC within 3 weeks of signing up. The application tracking made all the difference.</div>
      <div class="t-author">
        <div class="t-avatar">R</div>
        <div>
          <div class="t-name">Rahul Sharma</div>
          <div class="t-role">Software Engineer · Placed via HireFlow</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="subtitle">Sign in to your HireFlow account</div>
    ...
    <form action="${pageContext.request.contextPath}/login" method="post" id="loginForm" novalidate>
      ...
      <div class="form-group">
        ...
        <div class="field-err" id="pwdErr">Password is required.</div>
      </div>

      <div class="remember-row">
        <label class="check-label">
          <input type="checkbox" name="rememberMe" value="true"/> Remember me for 30 days
        </label>
    ...
    <div class="bottom-link">Don't have an account? <a href="${pageContext.request.contextPath}/register">Create one free</a></div>
  </div>
</div>
...
function validatePwd(){
  const i=document.getElementById('password'),e=document.getElementById('pwdErr'),ok=i.value.trim().length>0;
...
document.getElementById('loginForm').addEventListener('submit',function(e){
  if(!validateEmail()||!validatePwd()){e.preventDefault();return;}
